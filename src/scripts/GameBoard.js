...
export default class GameBoard {
    ...
    #addTile() {
        ...
        if (
            Math.floor(Math.random() * (FOUR_TILE_SPAWN_CHANCE + 1)) ==
            FOUR_TILE_SPAWN_CHANCE
        ) {
    ...
    #calculateMove(direction) {
        ...
        for (let i = 0; i < 4; i++) {
            for (let j = 0; j < 4; j++) {
                ...
                let nextPos = [
                    currentTile[0] - direction[0],
                    currentTile[1] + direction[1]
                ];
                ...
                for (
                    ...
                    ;
                    nextPos = [
                        currentTile[0] - direction[0] * ++k,
                        currentTile[1] + direction[1] * k
                    ]
                ) {
                    ...
                    // Объединяем плитки, если они равны по значению
                        ...
                        vTiles[nextPos[0]][nextPos[1]] =
                            vTiles[currentTile[0]][currentTile[1]] * 2;
                        vTiles[currentTile[0]][currentTile[1]] = 0;
        ...
        animate({
            ...
            draw(progress) {
                for (let move of moves) {
                    ...
                    board.tiles[move[0][0]][move[0][1]].cell.style[direction] =
                        progress * totalDistance + 'px';
                }
                ...
                animate({
                    ...
                    draw(progress) {
                        for (let move of moves) {
                            ...
                            board.tiles[move[0][0]][
                                move[0][1]
                            ].cell.style.transform =
                                'scale(' + progress * ANIMATION.scale + ')';
                        }
    ...
    #Tile(cell) {
        ...
        tile.set = function (value) {
            ...
            this.cell.style.backgroundColor =
                COLORS[this.value ? Math.log2(this.value) : 0];
            this.cell.textContent = this.value ? this.value : '';
            this.cell.classList = this.value
                ? `tile tile-${this.value}`
                : 'tile';
        };
